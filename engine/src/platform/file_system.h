...
namespace caliope {
	...
	CE_API bool file_system_open(std::string& path, file_modes mode, file_handle& out_handle);

	CE_API void file_system_close(file_handle& handle);

	CE_API bool file_system_size(file_handle& handle, uint64& out_size);

	CE_API bool file_system_read_text(file_handle& handle, uint64 max_length, std::string& line_buf);
